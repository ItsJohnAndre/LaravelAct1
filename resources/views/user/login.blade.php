@include ('partials.header')
    <header class=max-w-lg mx-auto>
        <a href="#"></a>
            <h1 class="text-4xl font-bold text-indigo-700 text-center" >Student Login</h1>
    </header>
<main class="bg-white max-w-lg mx-auto p-8 my-10 rounded-lg shadow-2xl">
    <section>
        <h3 class="font-bold text-2xl">Welcome to Student System</h3>
        <p class=text-gray-600 pt-2>Sign up a new account
            <a href="/register" class="text-purple-800 font-bold">here</a>
        </p>
    </section>
    <section class="mt-10">
        <form action="/login/process" method="POST" class="flex flex-col">
            @csrf
            @error('email')
                <p class="text-red-300 text-xs p-1">
                    {{$message}}
                </p>
            @enderror
